#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#define TAM 21

void printar(double A[TAM][TAM], double B[TAM]){

    int i,j;
    for(i=0;i<TAM;i++){
        for(j=0;j<TAM;j++){
            printf(" %5.2lf ", A[i][j]);
        }
        printf("= %5.2lf ", B[i]);
        printf("\n");
    }
    printf("\n");
    printf("------------\n\n");
}

void SystemResol(double A[TAM][TAM], double B[TAM]){

    int i,j;
    double x[TAM];

    for(i=0;i<TAM;i++){
         x[i] = 0.0;  // inicializando vetor x zerado
    }

    for(i=TAM-1;i>=0;i--){
        double bi = B[i];
        if(i==TAM-1){
           x[i] = B[i] / A[i][i]; 
        }
        for(j=TAM-1;j>=0;j--){
            if(j!=i){
                bi -= A[i][j]*x[j];
            }
        }
        x[i] = bi / A[i][i];
        printf("x%d = %4.2lf\n", i+1, x[i]);

    }
}

void GaussParcial(double A[TAM][TAM], double B[TAM]){

    int i=0,j=0, N, M, linha=0;
    double multi[TAM][TAM];

    for(M=0;M<TAM;M++){
        for(N=0;N<TAM;N++){
            multi[M][N] = 0;
        }
    }

    for(j=0;j<TAM-1;j++){
        double MaiorValor=0.0;
        for(i=j;i<TAM;i++){
            double X = abs(A[i][j]);
            if(X > MaiorValor){ //achando o maior valor da coluna
                MaiorValor = A[i][j];
                linha = i;
            }
        }
        int ii, jj;
        for(ii=j;ii<j+1;ii++){
            double aux;
            for(jj=0;jj<TAM;jj++){
                aux = A[ii][jj];
                A[ii][jj] = A[linha][jj];   //Trocando linhas de A
                A[linha][jj] = aux;
            }

            aux = B[ii];
            B[ii] = B[linha];   //trocando linhas de B
            B[linha] = aux;
        }

                int m,n, N;
                for(m=j+1;m<TAM;m++){
                    multi[m][j] = -A[m][j]/A[j][j];

                    for(n=j;n<TAM;n++){
                        A[m][n] += multi[m][j] * A[j][n];
                        
                    }
                   B[m] += multi[m][j] * B[j];
                }
    }
    printf("\n\nMATRIZ COM PIVOTEAMENTO PARCIAL\n");
    printar(A, B);
    SystemResol(A,B);
    
}
int main(){

double A[TAM][TAM];
int soma = 1, i,j;

//montando matriz A
for(i=0;i<TAM;i++){
    for(j=0;j<TAM;j++){
        if(i<j){
            A[i][j] = soma;
            soma++;
        }
        if(i==j){
            A[i][j] = 0;
        }
    }
    soma=1;
}
soma=1;
for(j=0;j<TAM;j++){
    for(i=0;i<TAM;i++){
        if(i>j){
            A[i][j] = soma;
            soma++;
        }
    }
    soma=1;
}
//montando Vetor B
double B[TAM];
soma=1;
for(i=0;i<TAM;i++,soma++){
    B[i] = soma;
}

    printf("\n\nMATRIZ AUMENTADA\n");
    printar(A, B);
    GaussParcial(A, B);

    return 0;
}